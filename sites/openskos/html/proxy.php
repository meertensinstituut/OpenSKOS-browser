...
if ( CSAJAX_FILTERS ) {
	...
	if ( CSAJAX_FILTER_DOMAIN ) {
		if ( !in_array( $parsed['host'], $valid_requests ) ) {
			csajax_debug_message( 'PHP proxy error: invalid domain - ' . $parsed['host'] . ' is not included in valid requests' );
			exit;
		}
	} else {
		...
		//if ( !in_array( $check_url, $valid_requests ) ) {
                if ( !validateRequest($valid_requests, $check_url) ) {
			csajax_debug_message( 'PHP proxy error: invalid domain - ' . $request_url . ' is not included in valid requests' );
			exit;
		}
	}
}
